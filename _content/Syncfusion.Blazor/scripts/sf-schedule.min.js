...
window.sfBlazor.Schedule = function () {
  ...
  function (_super) {
    ...
    MonthEvent.prototype.renderAppointments = function () {
      ...
      if (this.parent.options.currentView === 'Month') {
        if (this.parent.options.rowAutoHeight && this.parent.activeViewOptions.group.resources.length === 0) {
      ...
      for (var i = 0; i < elementList.length; i++) {
        ...
        if (sf.base.isNullOrUndefined(cellHeights[rowIndex])) {
          cellHeights[rowIndex] = cellTd.getBoundingClientRect().height;
          cellTops[rowIndex] = cellTd.offsetTop;
        }
        ...
        var cellHeight = cellHeights[rowIndex];
        var cellIndex = cellTd.cellIndex;

        if (sf.base.isNullOrUndefined(cellLefts[cellIndex])) {
          cellLefts[cellIndex] = cellTd.offsetLeft;
        }

        if (this.parent.options.enableRtl) {
          appRight = cellTd.parentElement.offsetWidth - cellLefts[cellIndex] - this.cellWidth;
        } else {
          appLeft = cellLefts[cellIndex];
        }
        ...
        if (this.day >= 0) {
          ...
          if (!ele.classList.contains('e-more-indicator')) {
            if (!ele.classList.contains('e-block-indicator')) {
              ...
              if (ele.classList.contains('e-appointment') && sf.base.isNullOrUndefined(ele.__eventList)) {
                this.parent.eventBase.wireAppointmentEvents(ele);
              }
              ...
              if (this.parent.options.rowAutoHeight) {
                ...
                if (sf.base.isNullOrUndefined(cellHeights[resIndex])) {
                  ...
                  ele.parentElement.style.height = cellHeights[resIndex] + 'px';
                }
  ...
  function (_super) {
    ...
    Month.prototype.renderLayout = function () {
      ...
      if (this.parent.options.currentView === 'Month' || this.parent.options.currentView === 'MonthAgenda') {
        ...
        var headerElem = [].slice.call(this.parent.element.querySelectorAll('.' + HEADER_CELLS_CLASS));
        var isByDate = this.parent.activeViewOptions != null && this.parent.activeViewOptions.group.byDate && this.parent.options.currentView === 'Month';
        var colSpan = void 0;

        if (isByDate) {
          colSpan = parseInt(headerElem[0].getAttribute('colspan'), 10);
        }

        for (var _i = 0, curEle_1 = curEle; _i < curEle_1.length; _i++) {
          var ele = curEle_1[_i];
          var index = ele.cellIndex;
          index = isByDate ? Math.floor(index / colSpan) : index;
          var curHeader = headerElem[index];
  ...
  function (_super) {
      ...
      if (this.parent.options.currentView === 'TimelineYear') {
        ...
        containerEle.forEach(function (element, i) {
          if (element.classList.contains(HIDDEN_CLASS)) {
            ...
            element.style.height = trElement_1[i].getBoundingClientRect().height + 'px';
          }
        });
        ...
        for (var i = 0; i < appointments.length; i++) {
          ...
          if (!sf.base.isNullOrUndefined(cellData)) {
            if (ele.classList.contains('e-appointment')) {
              cellTop = cellHeader + eventHeight * levelIndex + EVENT_GAP;
              ...
              if (this.parent.options.rowAutoHeight && cellHeight < availableHeight) {
                ...
                if (sf.base.isNullOrUndefined(cellHeights[rowIndex])) {
                  cellHeights[rowIndex] = Math.ceil(trEle.getBoundingClientRect().height);
                  ele.parentElement.style.height = cellHeights[rowIndex] + 'px';
                }

                this.updateCellHeight(cellData, availableHeight, ele, cellHeights[rowIndex]);
              }
            } else {
    ...
    Year.prototype.updateCellHeight = function (cell, height, appointment, cellHeight) {
      if (height > cellHeight) {
        sf.base.setStyleAttribute(cell.parentElement.querySelector('td'), {
          'height': height + 'px'
        });
        sf.base.setStyleAttribute(appointment.querySelector('td'), {
          'height': height + 'px'
        });
        ...
        if (this.parent.activeViewOptions.group.resources.length > 0) {
          ...
          var appContainer = this.parent.element.querySelector('.' + APPOINTMENT_CONTAINER_CLASS + '[data-group-index="' + cell.getAttribute('data-group-index') + '"]');
  ...
  function (_super) {
    ...
    DragAndDrop.prototype.calculateResourceGroupingPosition = function (e, cloneElement) {
      ...
      var resIndex = this.parent.virtualScrollModule ? rowIndex : this.actionObj.groupIndex;
      var eventContainer = this.parent.element.querySelectorAll('.e-appointment-container:not(.e-hidden)').item(resIndex);
  ...
  function () {
    ...
    VirtualScroll.prototype.updateContent = function () {
      if (this.parent.activeViewOptions.enableVirtualMask) {
      ...
      } else if (this.parent.options.currentView !== 'TimelineYear') {
        ...
        if (this.isDownScroll && !(firstIndex <= this.startIndex && this.startIndex <= lastIndex) || !this.isDownScroll && this.startIndex + this.renderedLength <= firstIndex && this.startIndex + this.renderedLength <= lastIndex) {
          for (var i = 0; i < containerEle.length; i++) {
            var eleColl = [].slice.call(containerEle[i].children);
            eleColl.forEach(function (ele) {
              ele.style.visibility = 'hidden';
            });
          }
  ...
  var Schedule = {
    ...
    updateLayout: function updateLayout(dataId, options, viewOptions) {
      options.selectedDate = new Date(options.selectedDate);
      options.minDate = new Date(options.minDate);
      options.maxDate = new Date(options.maxDate);
      ...
      if (instance && instance.element) {
        if (instance.isTimelineView() && instance.options.selectedDate !== options.selectedDate) {
          instance.uiStateValues.isInitial = true;
        }
    ...
    printWithOptions: function printWithOptions(dataId, dotnetRef, printOptions) {
      ...
      if (instance && instance.element) {
        ...
        new SfSchedule(null, cloneEle, options, viewOptions, dotnetRef, true);

        if (options.currentView !== 'MonthAgenda') {
          instance.dataReady(undefined, undefined);
        }

        instance.printScheduler(cloneEle, left, top_1);
        instance.destroy();
      }
    },
...
}]);(async()=>{await import("/_content/Syncfusion.Blazor/scripts/syncfusion-blazor-base.min.js").then(()=>{sfBlazor.loadDependencies('sfschedule');})})();