...
window.sfBlazor.BulletChart = function () {
  ...
  function () {
    ...
    SfBulletChart.prototype.barAnimation = function (data, barElement, type, valueX, valueY) {
      ...
      animation.animate(barElement, {
        duration: data.duration <= 0 && sf.base.animationMode === 'Enable' ? 1000 : data.duration,
        delay: data.delay,
  ...
  var BulletChart = {
    ...
    doBarAnimation: function doBarAnimation(element, data, id, type, valueX, valueY) {
      for (var i = 0; i < id.length; i++) {
        var barElement = document.getElementById(id[i]);

        if (barElement) {
          type === 'valueBarAnimation' ? element.blazor__instance.barAnimation(data, barElement, type, valueX[i]) : element.blazor__instance.barAnimation(data, barElement, type, valueX[i], valueY[i]);
        }
      }
    },
...
}]);(async()=>{await import("/_content/Syncfusion.Blazor/scripts/syncfusion-blazor-base.min.js").then(()=>{sfBlazor.loadDependencies('sfbulletchart');})})();