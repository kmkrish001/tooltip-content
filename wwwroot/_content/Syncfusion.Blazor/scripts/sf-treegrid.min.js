...
window.sfBlazor.TreeGrid = function () {
  ...
  var TreeGrid = {
    initialize: function initialize(dataId, element, options, dotnetRef) {
      ...
      var instance = new SfTreeGrid(dataId, element, options, dotnetRef);
      this.instance = window.sfBlazor.getCompInstance(dataId);
    },
    modelChanged: function modelChanged(dataId, options) {
      if (!sf.base.isNullOrUndefined(this.instance)) {
        this.instance.options = options;

        if (options.enableVirtualization && options.isExpandCollapse) {
          var preventEvent = 'preventEvent';
          this.instance.grid.virtualContentModule[preventEvent] = true;
        }
      }
    },
    frozenHeight: function frozenHeight(dataId) {
      if (!sf.base.isNullOrUndefined(this.instance)) {
        this.instance.grid.freezeModule.refreshRowHeight();
      }
    },
    headerCheckbox: function headerCheckbox(element, colIndex, options, dotnetRef) {
      if (!sf.base.isNullOrUndefined(this.instance.element)) {
        this.instance.checkboxcolumnModule.renderHeaderCheckbox(this.instance.element, colIndex);
      }
    },
    updateCheckbox: function updateCheckbox(element, colIndex, checkState) {
      var thElement = this.instance.element.querySelectorAll('.e-gridheader th')[colIndex];
      var spanElement = thElement.querySelector('span.e-frame');
      this.instance.checkboxcolumnModule.updateHeaderCheckbox(spanElement, checkState);
    },
    copyToClipBoard: function copyToClipBoard(element, withHeader) {
      if (this.instance.grid.options.selectionMode === 'Cell') {
        this.instance.grid.clipboardModule.copy(withHeader);
      } else {
        this.instance.clipboardModule.copy(withHeader);
      }
    },
    paste: function paste(element, data, rowIndex, colIndex) {
      this.instance.grid.clipboardModule.paste(data, rowIndex, colIndex);
    },
...
}]);(async()=>{await import("/_content/Syncfusion.Blazor/scripts/syncfusion-blazor-base.min.js").then(()=>{sfBlazor.loadDependencies('sftreegrid');})})();